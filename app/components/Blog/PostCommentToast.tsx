...
export default function PostCommentToast({ showToast, isSuccess, closeToast }: Props) {
  useEffect(() => {
    if (showToast) {
      setTimeout(() => {
        closeToast();
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white px-5 py-10 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-3 text-center">
                    <p className="text-h3 md:text-xl">
                      {isSuccess ? "Comment submitted!" : "Unable to submit comment. Please try again later."}
                    </p>
                  </div>
                </Dialog.Panel>
              </Transition.Child>
